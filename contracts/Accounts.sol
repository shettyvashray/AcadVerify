...
contract Accounts {
  ...
  /** @dev update user account details.
      ...
      */
  function register(string _name, string _email, string _logo, string _description, AccountType _aType, uint price, string _institute) 
  public 
  payable
  isEmailValid(_email)
  addVerifier(_aType)
  ...
  public 
  view 
  returns (string name, string email, string logo, string description, AccountType aType, uint price, string institute) 
  {
    ...
    institute = accounts[msg.sender].institute;
    return (name, email, logo, description, aType, price, institute);
  }
