  ...
  return function(dispatch) {
    ...
    ipfs.add(payload.logo)
      .then((res) => {
        ...
        // payload.institute = payload.institute || '';
        return contract.accounts.register.estimateGas(payload.name, payload.email, ipfsHash, payload.description, payload.type, payload.price, payload.institute, {from: payload.account});
      })
      .then((gas)=>{
        return contract.accounts.register(payload.name, payload.email, ipfsHash, payload.description, payload.type, payload.price, payload.institute, {from: payload.account, gasLimit: gas});
      })
      .then(()=>{
